  ...
  return (
    <div className="relative min-h-screen overflow-hidden transition-colors duration-500" style={{ fontFamily: 'MaoKenZhuyuanTi, sans-serif' }}>
      ...
      <div className="fixed top-6 right-6 z-[500] flex items-center gap-2">
        <Link href="/download" className="px-5 py-2.5 bg-white text-black rounded-xl font-bold text-[0.625rem] sm:text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-2 shadow-xl border border-black/5"><Download size={14} /> WENBrowser</Link>
      ...
      <div className="fixed top-6 left-6 z-[100] flex items-center gap-3"><h1 className={`text-[1.25rem] font-bold tracking-tighter italic hidden sm:block ${currentTheme.text}`}>WENBrowser</h1></div>
        ...
        {isLinkModalOpen && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
             ...
             <motion.div key="link-modal-content" initial={{opacity:0, scale: 0.9, y: 20}} animate={{opacity:1, scale: 1, y: 0}} exit={{opacity:0, scale: 0.9, y: 20}} className={`relative ${currentTheme.panel} border ${currentTheme.panelBorder} rounded-[2rem] w-full max-sm:max-w-xs max-w-sm overflow-hidden shadow-2xl shadow-black`}>
               <div className={`px-8 py-8 border-b ${currentTheme.panelBorder} flex items-center gap-3`}><div className={`p-2 ${currentTheme.panelSidebar} rounded-lg`}><Plus size={16} className="opacity-60"/></div><div><h3 className={`font-bold text-[1.125rem] leading-none ${currentTheme.panelText}`}>添加导航节点</h3><p className={`text-[0.5625rem] opacity-60 uppercase tracking-widest mt-1 ${currentTheme.panelMuted}`}>New Shortcut node</p></div></div>
               <div className="p-8 space-y-6"><div className="space-y-4"><div className="space-y-1.5"><label className={`text-[0.5625rem] font-bold opacity-80 uppercase tracking-[0.2em] pl-1 ${currentTheme.panelText}`}>名称 / LABEL</label><input autoFocus value={linkModalData.name} onChange={e=>setLinkModalData(p=>({...p, name: e.target.value}))} placeholder="网站名称" className={`w-full ${currentTheme.panelInput} border ${currentTheme.panelBorder} rounded-xl px-5 py-4 text-[0.875rem] font-bold focus:outline-none focus:border-cyan-500/50 transition-all`} /></div><div className="space-y-1.5"><label className={`text-[0.5625rem] font-bold opacity-80 uppercase tracking-[0.2em] pl-1 ${currentTheme.panelText}`}>链接 / URL</label><input value={linkModalData.url} onChange={e=>setLinkModalData(p=>({...p, url: e.target.value}))} placeholder="https://" className={`w-full ${currentTheme.panelInput} border ${currentTheme.panelBorder} rounded-xl px-5 py-4 text-[0.875rem] font-bold focus:outline-none focus:border-cyan-500/50 transition-all`} /></div></div><div className="grid grid-cols-2 gap-3 pt-2"><button onClick={()=>setIsLinkModalOpen(false)} className={`py-4 ${currentTheme.panelSidebar} border ${currentTheme.panelBorder} rounded-xl text-[0.6875rem] font-bold hover:brightness-110 transition-all ${currentTheme.panelText}`}>取消退出</button><button onClick={()=>{const { catId, name, url } = linkModalData; if (!name || !url) return; setCategories(prev => prev.map(cat => cat.id === catId ? { ...cat, links: [...cat.links, { id: `link-${Date.now()}`, name, url }] } : cat)); setIsLinkModalOpen(false); }} className={`py-4 ${currentTheme.accent} rounded-xl text-[0.6875rem] font-bold shadow-xl active:scale-95 transition-all text-white`}>保存节点</button></div></div>
             </motion.div>
          </div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isCatModalOpen && (
           <div className="fixed inset-0 z-[305] flex items-center justify-center p-4">
              ...
              <motion.div key="cat-modal-content" initial={{opacity:0, scale: 0.9, y: 20}} animate={{opacity:1, scale: 1, y: 0}} exit={{opacity:0, scale: 0.9, y: 20}} className={`relative ${currentTheme.panel} border ${currentTheme.panelBorder} rounded-[2rem] w-full max-w-sm overflow-hidden shadow-2xl`}><div className={`px-8 py-8 border-b ${currentTheme.panelBorder} flex items-center gap-3`}><div className={`p-2 ${currentTheme.panelSidebar} rounded-lg`}><FolderPlus size={16} className="opacity-60"/></div><div><h3 className={`font-bold text-[1.125rem] leading-none ${currentTheme.panelText}`}>新建分类层</h3><p className={`text-[0.5625rem] opacity-60 uppercase tracking-widest mt-1 ${currentTheme.panelMuted}`}>Add Module Cluster</p></div></div><div className="p-8 space-y-6"><div className="space-y-2"><label className={`text-[0.5625rem] font-bold opacity-80 uppercase tracking-[0.2em] pl-1 ${currentTheme.panelText}`}>分类名称 / CLUSTER TITLE</label><input autoFocus value={newCatTitle} onChange={e=>setNewCatTitle(e.target.value)} placeholder="例如：娱乐天地" className={`w-full ${currentTheme.panelInput} border ${currentTheme.panelBorder} rounded-xl px-5 py-4 text-[0.875rem] font-bold focus:outline-none focus:border-cyan-500/50 transition-all`} /></div><div className="grid grid-cols-2 gap-3 pt-2"><button onClick={()=>setIsCatModalOpen(false)} className={`py-4 ${currentTheme.panelSidebar} border ${currentTheme.panelBorder} rounded-xl text-[0.6875rem] font-bold hover:brightness-110 transition-all ${currentTheme.panelText}`}>放弃操作</button><button onClick={()=>{if (!newCatTitle.trim()) return; const nI = `cat-${Date.now()}`; setCategories([...categories, { id: nI, title: newCatTitle, links: [] }]); setSelectedCatId(nI); setNewCatTitle(""); setIsCatModalOpen(false);}} className={`py-4 ${currentTheme.accent} rounded-xl text-[0.6875rem] font-bold shadow-xl active:scale-95 transition-all text-white`}>立即创建</button></div></div></motion.div>
           </div>
        )}
      </AnimatePresence>
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden">
                    {selectedCatId && categories.find(c=>c.id===selectedCatId) ? (
                          ...
                          <div className="grid grid-cols-1 gap-4">
                            ...
                            {categories.find(c=>c.id===selectedCatId)?.links.map((l, lIdx) => (
                              <motion.div key={l.id} className={`flex gap-4 p-5 rounded-[1.6rem] items-center border transition-all ${currentTheme.panelInput} ${currentTheme.panelBorder}`}>
                                ...
                                <input value={l.name} onChange={(e)=>{const n=[...categories]; const ci=n.findIndex(x=>x.id===selectedCatId); n[ci].links[lIdx].name=e.target.value; setCategories(n);}} className={`bg-transparent text-[0.875rem] font-bold w-44 focus:outline-none ${currentTheme.panelText}`} />
                                <input value={l.url} onChange={(e)=>{const n=[...categories]; const ci=n.findIndex(x=>x.id===selectedCatId); n[ci].links[lIdx].url=e.target.value; setCategories(n);}} className={`bg-transparent text-[0.6875rem] flex-1 opacity-60 focus:opacity-100 transition-opacity truncate focus:outline-none ${currentTheme.panelMuted}`} />
                            ...
                            <button onClick={()=>{ setLinkModalData({ catId: selectedCatId!, name: '', url: 'https://' }); setIsLinkModalOpen(true); }} className={`w-full py-6 border-2 border-dashed rounded-[1.6rem] flex items-center justify-center gap-3 text-[0.6875rem] font-bold opacity-60 hover:opacity-100 transition-all mt-4 ${currentTheme.panelBorder} ${currentTheme.panelText}`}><Plus size={18}/> APPEND NEW NODE</button>
                          </div>
                        </div>
                      </>
                    ) : (
                      <div className={`flex-1 flex flex-col items-center justify-center text-center opacity-20 p-20 ${currentTheme.panelText}`}>
                        ...
                        <h4 className="text-[1.25rem] font-bold uppercase tracking-widest">请选择或创建一个分类模块</h4>
                      </div>
                    )}
                  </div>
               </div>
